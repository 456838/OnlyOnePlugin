...
public class FileSystemActivity extends BaseActivity {
    ...
    private void loadFiles(final File parent) {
        ...
        Disposable subscription = Observable.just(parent)
                .flatMap(new Function<File, ObservableSource<List<FileWrapper>>>() {
                    @Override
                    public ObservableSource<List<FileWrapper>> apply(File file) throws Exception {
                        ...
                        for (File _file : files) {
                            fileWrappers.add(new FileWrapper(_file));
                        }
